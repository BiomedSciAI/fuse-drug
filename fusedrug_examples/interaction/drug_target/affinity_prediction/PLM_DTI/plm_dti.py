...
class PLM_DTI_Module(pl.LightningModule):
    def __init__(self, cfg) -> None:
        ...
        if cfg.trainer.loss is None:
            ...
            if cfg.trainer.loss.lower() in ('bce','cross_entropy', 'ce','cross entropy'):
                loss_fct = torch.nn.BCELoss()
            elif cfg.trainer.loss.lower() in ('focal', 'focal_loss', 'focal loss'):
                loss_fct = partial(losses.focal_loss, is_input_logits=False, \
    ...
    def save_preds_for_benchmark_eval(self, test_step_outputs):
        ...
        preds = torch.cat([item['preds'] for item in test_step_outputs], 0).cpu().numpy()
        ...
        df = pd.DataFrame(columns=['source_dataset_versioned_name', 'source_dataset_activity_id', 'pred'])
