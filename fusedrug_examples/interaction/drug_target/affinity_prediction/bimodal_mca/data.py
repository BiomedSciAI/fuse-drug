...
class AffinityDataModule(pl.LightningDataModule):
    def __init__(self,         
        ...
        if self.pytoda_wrapped_tokenizer:
            if self.pytoda_ligand_tokenizer_json is None:
                raise Exception('you need to set both (pytoda_wrapped_tokenizer + pytoda_ligand_tokenizer_json + pytoda_target_tokenizer_amino_acid_dict) or neither')
    ...
    def _create_pipeline_desc(self, is_training, drug_target_affinity_loader_op):
        ...
        _molecule_tokenizer_path = os.path.join(get_molecule_pretrained_tokenizer_path(), 'bpe_tokenizer_trained_on_chembl_zinc_with_aug_4272372_samples_balanced_1_1.json')
        molecule_pad_id = 3
        ...
        if self.pytoda_wrapped_tokenizer:
            ligand_tokenizer_op = Op_pytoda_SMILESTokenizer(
                dict(vocab_file=self.pytoda_ligand_tokenizer_json,
            ...
            ligand_tokenizer_op = FastTokenizer(_molecule_tokenizer_path, pad_length=self.ligand_padding_length, pad_id=molecule_pad_id)
...
class OpLoadActiveSiteAlignmentInfo(OpBase):
    ...
    def __call__(self, sample_dict: NDict, op_id: Optional[str]=None, key_in='data.input.protein_str', key_out='data.input.protein_str'):
        '''
        params
