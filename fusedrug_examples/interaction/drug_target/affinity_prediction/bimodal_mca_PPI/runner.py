...
import hydra
from utils import hydra_resolvers #examples.fuse_examples.interaction.drug_target.affinity_prediction.bimodal_mca_PPI.
...
print('CUDA_VISIBLE_DEVICES=', os.environ['CUDA_VISIBLE_DEVICES'] if 'CUDA_VISIBLE_DEVICES' in os.environ else None)
...
def session_manager_and_logger_setup(cfg:dict)->None:
    ...
    if os.path.isfile(session_num_file):
        ...
        print('WARNING: this run was initiated without the session manager. This is ok for debugging, but it will pollute your code directory with all sorts of files/dirs.\n'
        'Few tips:\n'
        ...
        print(f'Note: the debug session directory, in which artifacts like logs and checkpoints will be saved, is: {cfg_raw["paths"]["debug_session_dir"]}')
    ...
    if cfg_raw['clearml']['active']:
        if (cfg_raw['load_from_checkpoint'] is not None) and cfg_raw['clearml']['load_from_checkpoint_continues_within_session']:
            print(Fore.GREEN+f'for clearml: continuing existing task - project_name={project_name}, task_name={task_name}, continue_last_task=True')
            task = Task.init(project_name=project_name, task_name=task_name,
                continue_last_task=True,
            )
        else:
            print(Fore.GREEN+f'for clearml: starting a new task - project_name={project_name}, task_name={task_name}, reuse_last_task_id=False')
            task = Task.init(project_name=project_name, task_name=task_name,
...
def main(cfg : DictConfig) -> None:
    ...
    if ('ONLY_GET_EXPECTED_WORKING_DIR' in os.environ) or cfg.only_get_expected_working_dir:
        ...
        print(f'SESSION_FULL_PATH_QUERY_ANSWER={QUERY_SEP_TOKEN}{SESSION_FULL_PATH}{QUERY_SEP_TOKEN}') #this is used by "run_detached_fuse_based.sh" to know the working dir, to be able to create it and redirect stdour/err to it
        return 0
    print('Hydra config:')
