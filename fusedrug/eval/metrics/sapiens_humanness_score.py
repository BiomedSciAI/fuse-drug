...
def calculate_sapiens_humanness_mean_score(sequence: str) -> Tuple[float, str]:
    ...
    if not isinstance(sequence, str) or (len(sequence) == 0):
        print(f"calculate_sapiens_humanness_mean_score::ERROR: sequence is not string! sequence={sequence}")
    ...
    chain = Chain(
        sequence, name="dummy", scheme=humanization_params.scheme, cdr_definition=humanization_params.cdr_definition
    )
