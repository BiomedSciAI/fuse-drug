...
class DTIDataset:
    ...
    def dynamic_pipeline(
        data: pd.DataFrame, drug_fixed_size: int, target_fixed_size: int
    ) -> PipelineDefault:
        """
        :param data:
        ...
        dynamic_pipeline = [
            ...
            (
                OpReadDataframe(
                    data=data, rename_columns=rename_columns, key_column=None
                ),
                dict(),
            ),
            # pytoda tokenization
            (
                ...
                dict(
                    key_in="data.drug.smiles", key_out_tokens_ids="data.drug.tokenized"
                ),
            ),
            (
                Op_pytoda_ProteinTokenizer(amino_acid_dict="iupac"),
                dict(
                    key_in="data.target.sequence",
                    key_out_tokens_ids="data.target.tokenized",
                ),
            ),
            (
                OpToTensor(),
                dict(
                    key=["data.drug.tokenized", "data.target.tokenized"],
                    dtype=torch.int32,
                ),
            ),
    ...
    @staticmethod
    def dataset(
        data_path: str, drug_fixed_size: int = 40, target_fixed_size: int = 700
    ) -> DatasetDefault:
        """
        ...
        data = df[
            df["Target Sequence"].apply(DTIDataset._protein_seq_is_valid)
        ].reset_index()
        ...
        dataset = DatasetDefault(
            sample_ids=sample_ids, dynamic_pipeline=dynamic_pipeline
        )
        dataset.create()
