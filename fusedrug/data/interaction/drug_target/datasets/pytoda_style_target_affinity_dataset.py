...
class PytodaStyleDrugTargetAffinityDataset(Dataset):
    def __init__(self,
        ...
        if isinstance(self._ligands_smi, str):
            ...
            raise Exception('the provided ligands_smi is a table, you cannot provide ligands_indexed_text_table_kwargs for it')

        if not self._ligands_smi._allow_access_by_id:
            raise Exception('_allow_access_by_id is required for DrugTargetAffinityDataset ! please add _allow_access_by_id=True to ligands_indexed_text_table_kwargs')
        ...
        if isinstance(self._proteins_smi, str):
            ...
            raise Exception('the provided proteins_smi is a table, you cannot provide proteins_indexed_text_table_kwargs for it')

        if not self._proteins_smi._allow_access_by_id:
            raise Exception('_allow_access_by_id is required for DrugTargetAffinityDataset ! please add _allow_access_by_id=True to proteins_indexed_text_table_kwargs')

        self._affinity_pairs_csv_path = affinity_pairs_csv_path
        self._affinity_df = pd.read_csv(self._affinity_pairs_csv_path)

        self._affinity_df[affinity_pairs_csv_ligand_id_column_name] = self._affinity_df[affinity_pairs_csv_ligand_id_column_name].str.rstrip()
        self._affinity_df[affinity_pairs_csv_protein_id_column_name] = self._affinity_df[affinity_pairs_csv_protein_id_column_name].str.rstrip()
