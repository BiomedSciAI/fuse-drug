...
from fusedrug.data.interaction.drug_target.datasets.pytoda_style_target_affinity_dataset import PytodaStyleDrugTargetAffinityDataset
...
if __name__=='__main__':    
    ...
    pipeline_desc = [
        ...
        (affinity_loader, dict(key_out_ligand='data.input.ligand',
                            key_out_protein='data.input.protein',
                            key_out_ground_truth_affinity='data.gt.affinity_val')),
    ...
    def my_collate(samples, default_collate_keys:Optional[List[str]]=None):
        ...
        for s in samples:
            to_default_collate_sample, append_in_list_sample = _split_sample(s, default_collate_keys=default_collate_keys)
    ...
    collated_mb = my_collate([s1,s2,s3,s4], 
