...
def build_molecule_tokenizer(dataset:Union[Dataset,IterableDataset], output_tokenizer_json:str,
    batch_sampler:Optional[Sampler], num_workers:int=0, 
    full_cycles_num=None, iterations_num=None, time_limit_minutes=None, stop_filename=None):
    '''
...
@click.option('--output-tokenizer-json', default=None, help='path for output tokenizer json file which can be quickly loaded and used. If not provided, defaults to same as "INPUT_SMI_FILE" but with the extension modified to ".bpe_vocab.json"')
@click.option('--augment/--no-augment', default=True, help='When enabled activates augmentation when training the tokenizer.')
@click.option('--shuffle/--no-shuffle', default=True, help='When enabled activates shuffle of the dataset. This is useful especially in cases that a dataset is sorted, to avoid biased estimation of pairs during the beginning of the training.')
@click.option('--full-cycles-num', default=None, help='Sets the numbe of full cycles over the dataset. Useful when augment is activated in case you want to train on each sample multiple times. (mutually exclusive with iterations-num and with limit-time)')
@click.option('--iterations-num', default=None, help='Sets the number of iterations. (mutually exclusive with full-cycles-num and with limit-time)')
@click.option('--time-limit-minutes', default=None, help='limits the training to the defined amount of minutes. (mutually exclusive with full-cycles-num and with iterations-num)')
def main(input_smi_file:str, molecule_id_column_idx:int, output_tokenizer_json:Optional[str],
    augment:bool, shuffle:bool, full_cycles_num, iterations_num, time_limit_minutes):
    '''
