...
class SmilesRandomizeAtomOrder(OpBase):
    ...
    def __call__(self, sample_dict: NDict, key: str='data.input.ligand'):
        ...
        if not isinstance(mol, Chem.rdchem.Mol):
            ...
            raise Exception(f'sample_id={sid}: expected key_in={key} to point to RDKit mol, but instead got {type(mol)}. Note - you can use SmilesToRDKitMol Op to convert a smiles string to RDKit mol')        
