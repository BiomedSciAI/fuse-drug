    ...
    def __call__(self, sample_dict: NDict, key_in='data.input.ligand_str', key_out='data.input.ligand'):
        ...
        try:
            ...
            if self._verbose>0:
                ...
                print(f'ERROR in sample_id={sid}: The following smiles string could not be loaded by Chem.MolFromSmiles: {smiles_seq} - dropping sample.')
            return None
...
class RDKitMolToSmiles(OpBase):
    ...
    def __call__(self, sample_dict: NDict, key_in='data.input.ligand', key_out='data.input.ligand_str'):
        ...
        if not isinstance(mol, Chem.rdchem.Mol):
            raise Exception(f'expected key_in={key_in} to point to RDKit mol, but instead got {type(mol)}. Note - you can use SmilesToRDKitMol Op to convert a smiles string to RDKit mol')
...
class SanitizeMol(OpBase):
    ...
    def __call__(self, sample_dict: NDict, key):
        ...
        if not isinstance(mol, Chem.rdchem.Mol):
            raise Exception(f'expected key_in={key} to point to RDKit mol, but instead got {type(mol)}. Note - you can use SmilesToRDKitMol Op to convert a smiles string to RDKit mol')
        ...
        try:
            ...
            if self._verbose>0:
                sid = get_sample_id(sample_dict)
                print(f'The following smiles string did not pass SanitizeSmiles: {Chem.MolToSmiles(mol)} - dropping sample.')
            return None
