...
An example of adding special tokens to an existing tokenizer is found here: [ModularTokenizer update](test_multi_tokenizer_use.py#L35). Note, that these are instructions to add new tokens, not to remove existing ones.
...
* max_len: (Optional) maximum number of tokens encoded by each instance of this tokenizer. If not given or None - no limit is set. If max_len is defined both here and during a call to encode_list, the 
