...
def build_tokenizer(
    ...
    if override_post_processor is not None:            
        ...
        warn('no post_processor provided. This is usually ok if you are 1. Creating a tokenizer in order to store it in a file, as changing post-processor does not require retraining the tokenizer. 2. Intend to override the post-processor in the returned tokenizer.')
    ...
    if trainer is not None:        
        ...
        tokenizer.train_from_iterator(iterator_func(train_dataset=train_dataset, train_batch_sampler=train_batch_sampler, 
            full_cycles_num=full_cycles_num, iterations_num=iterations_num, time_limit_minutes=time_limit_minutes,
            stop_filename=stop_filename,
            num_workers=num_workers,
            ), trainer=trainer, length=len(train_dataset))
    
    if save_to_json_file is not None:
        tokenizer.save(save_to_json_file)
    
def iterator_func(train_dataset, train_batch_sampler, full_cycles_num, iterations_num, time_limit_minutes, 
    stop_filename, num_workers=0):            
...
class IsWorkDone:
    def __init__(self, 
        ...
        if iterations_num is not None:
            ...
            raise Exception('IsWorkDone::No mode selected! you must choose one of full_cycles_num, iterations_num or time_limit_minutes')
    ...
    def update(self, epoch_num, iteration_num) -> bool:
        ...
        if (self.stop_filename is not None) and (not iteration_num%1000):
            if os.path.isfile(self.stop_filename):
                print(f'stop filename = "{self.stop_filename}" encountered! stopping tokenizer training. After {iteration_num} iterations')
                return True
