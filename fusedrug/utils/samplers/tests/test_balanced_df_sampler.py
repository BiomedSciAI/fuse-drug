...
class TestBalancedClassDataFrameSampler(unittest.TestCase):
    def test_balanced_df_sampler(self):        
        ...
        df = pd.DataFrame({'index': ['sample'+str(i) for i in range(1000)], 'label': np.random.choice(['1', '2'], 1000, p=[0.8, 0.2]).tolist()})
        df.set_index("index", drop=True, inplace=True)
        sampler = BalancedClassDataFrameSampler(df=df, label_column_name="label", classes=['1', '2'], 
                                                counts=[5,5], shuffle=False, total_minibatches=1000)
        ...
        self.assertAlmostEqual(np.mean(np.diff(np.array(seen_all_1s_after))), seen_all_1s_after[0], delta=1)
        self.assertAlmostEqual(np.std(np.diff(np.array(seen_all_1s_after))), 0, delta=1)
        self.assertAlmostEqual(np.mean(np.diff(np.array(seen_all_2s_after))), seen_all_2s_after[0], delta=1)
