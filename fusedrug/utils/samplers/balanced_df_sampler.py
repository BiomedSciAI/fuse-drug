...
class BalancedClassDataFrameSampler(Sampler):
    ...
    def __init__(self, df: pd.DataFrame, label_column_name: str, classes:List[str], counts:List[int], shuffle:bool=True,
        ...
        for curr_class,curr_count in zip(self.classes, self.counts):
            ...
            minibatches_needed_to_see_all.append(int(np.ceil(indices.shape[0]/curr_count)))

        smallest_class_index = np.argmin([len(x) for x in self.indices_per_class])            

        total_minibatches_to_see_all_samples_at_least_once = minibatches_needed_to_see_all[smallest_class_index]
        print(f'To see all {self.indices_per_class[smallest_class_index].shape[0]} samples of the most rare class ({self.classes[smallest_class_index]}) it would take {total_minibatches_to_see_all_samples_at_least_once} minibatches.')
        print(f'set total_minibatches to "see_all_smallest_class" to use this number of minibatches.')
        if isinstance(total_minibatches, str):
            ...
            print(f'a custom total_minibatches was provided ({total_minibatches}). This is useful since if the number is large then something in pytorch (or pytorch lightning) makes GPU utilization drop drastically.')
            self.total_minibatches = total_minibatches

            epochs_to_see_all = total_minibatches_to_see_all_samples_at_least_once/total_minibatches
            print(f'Under the provided epoch size of {total_minibatches} minibatches per epoch, it would take {epochs_to_see_all:.2f} epochs to see at least once every sample.')
    ...
    def _get_next_sample(self, class_index):
        ...
        final_raw_index = self.indices_per_class[class_index][self.pointers[class_index]]
