...
class IndexedTextTable(Dataset):
    def __init__(self, 
        ...
        if self._columns_num_expectation is not None:
            assert self._id_column_idx < self._columns_num_expectation, f'self._id_column_idx={self._id_column_idx} is outside of range for the provided while self._columns_num_expectation={self._columns_num_expectation}'          
        ...
        if self._first_row_is_columns_names:
            ...
            if self._columns_names is None:
                ...
                print(f'Since columns_names was provided, using columns names = {self._columns_names} instead of {first_line_splitted}')
        ...
        print(f'column names used={self._columns_names} - if it does not look like column names make sure that the following args are properly set: first_row_is_columns_names, columns_names')                        
        ...
        if self._allow_access_by_id:
            ...
            if not ((id_column_idx is not None) ^ (id_column_name is not None)):
                raise Exception('allow_access_by_id is enabled, so you must provide exactly one of id_column_idx, id_column_name')
    ...
    def _split_line_basic(self, line_str) -> Tuple:
        ...
        if self._columns_num_expectation is not None:            
            if self._columns_num_expectation!=len(splitted):
                raise Exception(f'Expected each line to be {repr(self._seperator)} separated and contain exactly {self._columns_num_expectation} values, but instead got {len(splitted)} values : {splitted} Make sure that your smi file {self._filename} is in correct format.')
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            ...
            if not self._allow_access_by_id:
                raise Exception('allow_access_by_id was set to False, so you can not access by string id. pass allow_access_by_id=True if you are interested in this.')
