        ...
        **kwargs):
        ...
        for curr_func in self.process_funcs_pipeline:
            if not callable(curr_func):
                raise Exception(f'process_funcs_pipeline is expected to be a either None or a list of callables, but {curr_func} is not callable.')

        print('WARNING: DUPLICATES ARE NOT SEARCHED FOR YET!!!')
        self._fasta_file_loc = fasta_file_loc
        print("loading fasta file - note, if it's the first time you are loading this file, index building (using pyfastx) may take time ...")
        ...
        if self._check_for_duplicate_names:
            print('checking for duplicates (in sequences names! does not check the sequences content!!). May take few minutes...')
