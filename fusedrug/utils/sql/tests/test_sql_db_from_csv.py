...
class TestSQLfromCSV(unittest.TestCase):
    def test_single_chunk(self) -> None:
        ...
        small_csv_filepath = os.path.join(
            get_tests_data_dir(), "sql_from_csv", "small_input.tsv"
        )
    ...
    def test_many_chunks(self) -> None:
        large_csv_filepath = os.path.join(
            get_tests_data_dir(), "sql_from_csv", "large_input.tsv"
        )
        db_dir = os.path.join(get_tests_data_dir(), "sql_from_csv")
        db_name = "large_output_db"
        sql_creator = SQLfromCSV(db_dir=db_dir, db_name=db_name)
        sql_creator.add_table(
            input_filepath=large_csv_filepath,
            table_name="my_table",
            chunk_size=10,
            first_row_is_columns_names=True,
        )
        ...
        self.assertTrue(
            np.isclose(np.array(df["Float Column"]), np.linspace(1.1, 11.4, 104)).all()
        )
    ...
    def test_multiple_files(self) -> None:
        ...
        file_to_table_dict = {
            small_csv_filepath: "small_table",
            large_csv_filepath: "large_table",
        }
        ...
        self.assertTrue(
            np.isclose(np.array(df2["Float Column"]), np.linspace(1.1, 11.4, 104)).all()
        )
