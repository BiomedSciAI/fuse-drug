...
class TestSQLfromCSV(unittest.TestCase):
    def test_single_chunk(self):
        ...
        sql_creator.add_table(input_filepath=small_csv_filepath,
                            table_name='my_table', 
    ...
    def test_many_chunks(self):
        ...
        sql_creator = SQLfromCSV(db_dir=db_dir, db_name=db_name)
        sql_creator.add_table(input_filepath=large_csv_filepath, 
                            table_name='my_table', 
                            chunk_size=10,     
                            first_row_is_columns_names=True)
        query = 'select * from my_table'
    ...
    def test_multiple_files(self):
        ...
        small_csv_filepath = os.path.join(get_tests_data_dir(), 'sql_from_csv', 'small_input.tsv')
        large_csv_filepath = os.path.join(get_tests_data_dir(), 'sql_from_csv', 'large_input.tsv')
        file_to_table_dict = {small_csv_filepath: 'small_table', \
                              large_csv_filepath: 'large_table'}
        chunk_size_dict = {small_csv_filepath: 100000, \
                           large_csv_filepath: 10}
        first_row_is_columns_dict = {small_csv_filepath: True, \
                                      large_csv_filepath: True}

        db_dir = os.path.join(get_tests_data_dir(), 'sql_from_csv')
        db_name = 'multiple_tables_db'
        sql_creator = SQLfromCSV(db_dir=db_dir, db_name=db_name)
        for f in file_to_table_dict:
            sql_creator.add_table(input_filepath=f,
